...
def exchange_money(budget: float, exchange_rate: float) -> float:
    """
    Calculate amount of foreign currency to receive for a certain budget.
...
def get_change(budget: float, exchanging_value: float) -> float:
    """
    Calculate amount of budget that is not exchanged.
...
def get_value_of_bills(denomination: int, number_of_bills: int) -> int:
    """
    Calculate value of bills returned by exchange house.
...
def get_number_of_bills(amount: float, denomination: int) -> int:
    """
    Calculate numbers of bills that will be received.
...
def get_leftover_of_bills(amount: float, denomination: int) -> float:
    """
    Calculate value lost in exchange due to bill denomination.
...
def exchangeable_value(budget: float,
    ...
    # max_exchange_value = exchange_money(budget, actual_exchange_rate)
    # number_of_bills = get_number_of_bills(max_exchange_value, denomination)

    return get_value_of_bills(
        denomination,
        get_number_of_bills(
            exchange_money(budget, actual_exchange_rate),
            denomination
        )
    )
